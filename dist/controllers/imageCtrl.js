...
module.exports = {
    ...
    deleteImage: function deleteImage(req, res, next) {
        db.delete_image(req.params.id, function (error, response) {
            if (error) {
                res.json({
                    status: 500,
                    message: error,
    ...
    readImages: function readImages(req, res, next) {
        db.read_images_by_home_id(Number(req.params.home_id), function (error, response) {
            if (error) {
                res.json({
                    status: 500,
                    message: error,
